...
class SeuilCarousel {
  ...
  init() {
    if (!this.slides.length || !window.Swiper) {
      console.error('Swiper.js not available or no slides found');
      return;
    }
  ...
  validateStructure() {
    // S'assurer que le carousel a la classe swiper
    if (!this.carousel.classList.contains('swiper')) {
      this.carousel.classList.add('swiper');
    }

    // Vérifier le wrapper
    if (!this.wrapper) {
      console.error('swiper-wrapper not found in carousel');
      return;
    }

    // Vérifier que les slides ont la classe swiper-slide
    ...
    return {
      ...
      pagination: (this.options.showBullets && (needsNavigationDesktop || needsNavigationMobile)) ? {
        ...
        renderBullet: (index, className) => {
          return `<button class="${className}" aria-label="Aller à la slide ${index + 1}"></button>`;
        }
      } : false,
  ...
  onSlideChange(swiper) {
    // Index réel (important avec loop)
    this.currentIndex = this.options.loop ? swiper.realIndex : swiper.activeIndex;
    
    this.updateNavigation();
    this.updateTabs();
    this.updateSlideClasses();

    // Callback personnalisé (compatibilité avec ton ancien code)
    if (typeof this.options.onChange === 'function') {
      const currentSlide = this.slides[this.currentIndex];
      const currentPage = this.getCurrentPage();
      this.options.onChange(this.currentIndex, currentSlide, currentPage);
    }
  }

  updateNavigation() {
    if (!this.prevBtn || !this.nextBtn || !this.swiper) return;

    // Swiper gère automatiquement les classes disabled
    // On ajoute juste nos classes pour le style
    if (this.options.loop) {
      this.prevBtn.classList.remove('is-disabled');
      this.nextBtn.classList.remove('is-disabled');
    } else {
      this.prevBtn.classList.toggle('is-disabled', this.swiper.isBeginning);
      this.nextBtn.classList.toggle('is-disabled', this.swiper.isEnd);
    }
  }
  ...
  updateSlideClasses() {
    ...
    this.slides.forEach((slide, i) => {
      ...
      if (i >= this.currentIndex && i < this.currentIndex + slidesPerView) {
  ...
  // API publique (compatibilité avec ton ancien code)
  goToSlide(index) {
    if (this.swiper) {
...
class CarouselManager {
  ...
  get(carouselId) { return this.carousels.get(carouselId); }
